...
public class AdminControl extends AbstractControl<AdminForm> {
	 ...
	 *            le anneeScolaireService à modifier.
	 */
	public void setAnneeScolaireService(
	 ...
	 * 
	 * @param etablissementService
	 *            le etablissementService à modifier.
	 */
	...
	/**
	 * Si l'utilisateur est administrateur de ressource, il a alors accès à
	 * l'ensemble des onglets. S'il est adm local il a alors accès uniquement au
	 * paramétrage de son établissement.
	 */
	private void gestionAccesOnglet() {
		...
		final Boolean vraiOuFauxAccesAdmLoc = utilisateurDTO
				.getAdminLocalSiren().contains(sirenSel);
		final Boolean vraiOuFauxAccesAdmRess = utilisateurDTO
				.getAdminRessourceSiren().contains(sirenSel);
		final Boolean vraiOuFauxAdmRessEnt = utilisateurDTO
				.getVraiOuFauxAdmRessourceENT();
		final Boolean vraiOuFauxAdmCentral = utilisateurDTO
				.getVraiOuFauxAdmCentral();
		if (vraiOuFauxAccesAdmLoc || vraiOuFauxAccesAdmRess
				|| vraiOuFauxAdmRessEnt || vraiOuFauxAdmCentral) {
			if (vraiOuFauxAdmRessEnt || vraiOuFauxAdmCentral) {
				...
				mapAccesOnglet.put(TypeOngletAdmin.LIBELLES.name(), true);
				mapAccesOnglet.put(TypeOngletAdmin.OUVERTURE.name(), true);
				mapAccesOnglet.put(TypeOngletAdmin.PREFERENCES.name(), true);

				// selectionne le premier onglet accessible pour le profil si
				// l'étabblissement est fermé
				if (!BooleanUtils.isTrue(utilisateurDTO
						.getVraiOuFauxCahierOuvertEtab())) {
					form.setOngletSelectionne(TypeOngletAdmin.JOURS_OUVRES
							.name());
				} else {
				...
				if (!BooleanUtils.isTrue(form.getVraiOuFauxOuvertEntSource())) {
					MessageUtils.addMessage(new Message(
							TypeReglesAdmin.ADMIN_16.name()), getClass());
				}
			}
		} else {
			...
			mapAccesOnglet.put(TypeOngletAdmin.PREFERENCES.name(), false);
	...
	/**
	 * Sélectionne le mois depuis la barre de mois et génére le calendrier des
	 * jours chomés.
	 */
	public void selectionneBarreMois() {
		...
		form.setListeVancance(GenerateurDTO.genererVacanceCalendar(
				anneeScolaireDTO.getDateRentree(), anneeScolaireDTO
						.getDateSortie(), DateUtils.parse(form
						.getDateMoisSelectionne()), form
						.getListeJourOuvreGlobale(), GenerateurDTO
						.generateSetPeriodeVancanceFromDb(anneeScolaireDTO
								.getPeriodeVacances())));

		form.setListeMoisAnneeSco(GenerateurDTO.genererBarreMois(
	...
	public void saveAnneeScolaire() {
		try {
			anneeScolaireService.saveAnneeScolaire(form
					.getDateAnneeScolaireQO());
			// mise à jour du contexte utilisateur
			final AnneeScolaireDTO anneeScolaireDTO = obtenirSessionAnneeScolaireDTO();
			final DateAnneeScolaireQO dateAnneeScolaireQO = ObjectUtils
					.clone(form.getDateAnneeScolaireQO());
			anneeScolaireDTO.setDateRentree(dateAnneeScolaireQO
					.getDateRentree());
			anneeScolaireDTO.setDateSortie(dateAnneeScolaireQO.getDateSortie());

			// navigation
			form.setDateMoisSelectionne(DateUtils
					.formatDateAuPremierJourDuMois(dateAnneeScolaireQO
							.getDateRentree()));
			selectionneBarreMois();
		} catch (MetierException e) {
			log.debug("Erreur de sauvegarde de l'année scolaire : {}.",
					e.getMessage());
		}
	}
	...
	public void ajouterPlageFerie() {
		final Date dateDebut = (form.getDateDebutFerie() != null) ? ((Date) form
				.getDateDebutFerie().clone()) : null;
		final Date dateFin = (form.getDateFinFerie() != null) ? ((Date) form
				.getDateFinFerie().clone()) : null;
		try {
			final AnneeScolaireDTO anneeScolaireDTO = obtenirSessionAnneeScolaireDTO();

			// Check regle de non nullité et de cohérence des dates et de
			// chevauchement de plage.
			...
			datePeriodeQO.setPlageExistante(anneeScolaireDTO
					.getPeriodeVacances());
			...
			final List<GenericDTO<Date, Date>> listeClonee = ObjectUtils
					.clone(form.getListeJourFerie());
			...
			// navigation
			form.setDateMoisSelectionne(DateUtils
					.formatDateAuPremierJourDuMois(dateDebutMoisSel));
			selectionneBarreMois();
		} catch (MetierException e) {
	...
	public void supprimerPlageFerie() {
		...
		try {
			...
			// navigation
			form.setDateMoisSelectionne(DateUtils
					.formatDateAuPremierJourDuMois(form
							.getPlageJourFerieSelectionne().getValeur1()));
			selectionneBarreMois();
		} catch (MetierException e) {
			log.debug("Erreur de sauvegarde des périodes de vacances ", e);
		}
	 ...
	 * 
	 * @param listePlageIndispo
	 *            la nouvelle liste de plage.
	 * @throws MetierException
	 *             l'exception potentielle.
	 * 
	 */
	private void savePeriodeVacance(
			final List<GenericDTO<Date, Date>> listePlageIndispo)
			throws MetierException {

		final String chainePeriode = GenerateurDTO
				.generatePeriodeVacanceToDb(listePlageIndispo);
	...
	public void saveOuvertureENT() {
		...
		try {
			...
			log.debug(
					"Erreur de sauvegarde de l'ouverture aux établissements: {0}.",
					e.getMessage());
		}
	}
	...
	public void initialiseValeurEtGrilleHoraire() {
		final EtablissementComplementDTO etablissementComplementDTO = etablissementService
				.findDonneeComplementaireEtablissement(obtenirSessionUtilisateurDTO()
						.getIdEtablissement());

		form.setDureeCours(ObjectUtils.defaultIfNull(
				etablissementComplementDTO.getDureeCours(),
				form.getDureeCours()));
		form.setHeureDebut(ObjectUtils.defaultIfNull(
				etablissementComplementDTO.getHeureDebutCours(),
				form.getHeureDebut()));
		form.setMinuteDebut(ObjectUtils.defaultIfNull(
				etablissementComplementDTO.getMinuteDebutCours(),
				form.getMinuteDebut()));
		form.setHeureFin(ObjectUtils.defaultIfNull(
				etablissementComplementDTO.getHeureFinCours(),
				form.getHeureFin()));
		form.setFractionnement(etablissementComplementDTO.getFractionnement());

		if (etablissementComplementDTO.getHoraireCours() != null) {
			form.setHorairesCours(GenerateurDTO
					.generateGrilleHoraireFromDb(etablissementComplementDTO
							.getHoraireCours()));
		} else {
	...
	public void saveGrilleHoraire() {
		...
		etablissementQO.setId(obtenirSessionUtilisateurDTO()
				.getIdEtablissement());
		etablissementQO.setHoraireCours(GenerateurDTO
				.generateGrilleHoraireToDb(form.getHorairesCours()));
		...
		try {
			...
			log.debug("Erreur de sauvegarde de la grille horaire {0}",
					e.getMessage());
		}
	}

	/**
	 * Methode appele par le a4j:jsFunction lors de la sélection d'une plage à
	 * modifier / supprimer. Positionne sur le formulaire la plage à éditer.
	 */
	public void chargerPlageSelected() {
		...
		log.info(
				" chargerPlageSelected Selected index {0}, total grille {1}, debut {2}:{3}, fin {4}:{5}.  obj {6} obj {7}",
				form.getPlageSelectedIndex(), form.getHorairesCours().size(),
				form.getPlageEditee().getHeureDebut(), form.getPlageEditee()
						.getMinuteDebut(), form.getPlageEditee().getHeureFin(),
				form.getPlageEditee().getMinuteFin(), form.getHorairesCours()
						.get(form.getPlageSelectedIndex()), form
						.getPlageEditee());
	}

	/**
	 * Methode appele par le a4j:jsFunction lors de l'édition d'une plage à
	 * modifier / supprimer. Positionne sur le formulaire la plage à éditer :
	 * fait un clone.
	 */
	public void editerPlageSelected() {
		...
		final Integer index = form.getPlageSelectedIndex();
		final GrilleHoraireDTO plage = ObjectUtils.clone(form
				.getHorairesCours().get(index));
	 ...
	 * 
	 * @param plageSelectedIndex
	 *            le plageSelectedIndex to set
	 */
	...
	private void initialiserBarreJourOuvre() {
		form.setListeOuvre(GenerateurDTO
				.generateListeJourOuvreFerieFromDb(obtenirSessionUtilisateurDTO()
	...
	public void saveJourOuvre() {
		final List<GenericTroisDTO<String, String, Boolean>> listeJour = ObjectUtils
				.clone(form.getListeOuvre());

		// passe le jour a ouvré ou vaqué.
		for (final GenericTroisDTO<String, String, Boolean> jour : listeJour) {
			if (StringUtils.equals(jour.getValeur1(),
					form.getLibelleJourOuvreSel())) {
		...
		etablissementQO.setJoursOuvres(GenerateurDTO
				.generateJourOuvreToDb(listeJour));
		...
		try {
			...
			utilisateurDTO.setJoursOuvresEtablissement(etablissementQO
					.getJoursOuvres());
		} catch (MetierException e) {
			log.debug("Erreur de sauvegarde des jours ouvrés : {0}",
					e.getMessage());
		}
	}
	...
	private void initialiserBarreAlternanceSemaine() {
		final String alternanceSemaine = etablissementService
				.findAlternanceSemaine(obtenirSessionUtilisateurDTO()
						.getIdEtablissement());
	...
	public void activerAlternanceSemaine() {
		if (form.getVraiOuFauxAlternanceSemaine()) {
			// initialisation.
			final AnneeScolaireDTO anneeScolaireDTO = obtenirSessionAnneeScolaireDTO();
			form.setListeBarreSemaine(GenerateurDTO.generateListeSemaine(
					anneeScolaireDTO, "", true));
		} else {
			// désactivation.
			final AnneeScolaireDTO anneeScolaireDTO = obtenirSessionAnneeScolaireDTO();
			form.setListeBarreSemaine(GenerateurDTO.generateListeSemaine(
					anneeScolaireDTO, "", false));
	...
	public void saveAlternanceSemaine() {
		...
		final Boolean vraiOuFauxRepercution = form
				.getRepercutionModificationAlternance();

		if (!StringUtils.isEmpty(numeroSemSel)) {
			for (final BarreSemaineDTO barreSemaineDTO : form
					.getListeBarreSemaine()) {
				final TypeSemaine type = barreSemaineDTO.getTypeJourEmploi();
				if (StringUtils.equals(barreSemaineDTO.getNumeroSemaine(),
						numeroSemSel)) {
	 ...
	 * 
	 * @param typeJourEmploi
	 *            le type.
	 * @param barreSemaineDTO
	 *            la semaine.
	 */
	private void basculementAlternanceUneSemaine(
			final TypeSemaine typeJourEmploi,
			final BarreSemaineDTO barreSemaineDTO) {
		if (!TypeSemaine.VAQUE.equals(typeJourEmploi)) {
			if (TypeSemaine.IMPAIR.equals(typeJourEmploi)) {
				...
				barreSemaineDTO.setCouleur(GenerateurDTO
						.getMapAlternanceCouleur().get(TypeSemaine.PAIR));
			} else {
				barreSemaineDTO.setTypeJourEmploi(TypeSemaine.IMPAIR);
				barreSemaineDTO.setCouleur(GenerateurDTO
						.getMapAlternanceCouleur().get(TypeSemaine.IMPAIR));
			}
		}
	...
	public void saveAlternancesSemaines() {
		...
		etablissementQO.setId(obtenirSessionUtilisateurDTO()
				.getIdEtablissement());
		etablissementQO.setAlternanceSemaine(GenerateurDTO
				.generateAlternanceSemaineToDb(form.getListeBarreSemaine()));

		try {
			etablissementService.saveEtablissementAlternance(etablissementQO);
		} catch (MetierException e) {
			log.debug(
					"Erreur de sauvegarde de l'alternance des semaines : {0}",
					e.getMessage());
		}
	...
	public void initialisationListeLibelle() {
		...
		final Integer idEtablissement = obtenirSessionUtilisateurDTO()
				.getIdEtablissement();
		form.setListeEnseignement(enseignementService
				.findEnseignementEtablissement(idEtablissement));
	...
	private void initialiserListeLibelleTypeDevoir(final Integer idEtablissement) {
		form.setListeTypeDevoir(devoirService.findListeTypeDevoir(
	...
	private void initialiserPreferences() {
		...
		Set<TypePreferencesEtab> prefsEtab = preferencesService.findEtabPreferences(utilisateurDTO.getIdEtablissement());
		
		for (TypePreferencesEtab pref : prefsEtab) {
			switch (pref) {
			case SeancePartage:
				form.setPrefSeancePartage(true);
				break;
	...
	public void savePrefEtab(){
		final UtilisateurDTO utilisateurDTO = obtenirSessionUtilisateurDTO();
		Set<TypePreferencesEtab> prefsEtab = EnumSet.noneOf(TypePreferencesEtab.class);
		for (TypePreferencesEtab pref : TypePreferencesEtab.values()) {
			switch (pref) {
			case SeancePartage:
				if (form.isPrefSeancePartage()) {
					prefsEtab.add(pref);
				}
				break;
		...
		preferencesService.saveEtabPreferences(utilisateurDTO.getIdEtablissement(), prefsEtab);
	}
	/**
	...
	public void saveLibelleEnseignement() {
		...
		enseignementQO.setIdEtablissement(obtenirSessionUtilisateurDTO()
				.getIdEtablissement());
		...
		try {
			...
			form.getEnseignementSel().setLibellePerso(
					form.getLibelleEnseignement());
			form.setLibelleEnseignement(null);
		} catch (MetierException e) {
			log.debug(
					"Erreur de sauvegarde du libellé de l'enseignement : {0}",
					e.getMessage());
		}
	}
	...
	public void selectionnerLibelleTypeDevoir() {
		...
		form.setVraiOuFauxDevoir(TypeCategorieTypeDevoir.DEVOIR.equals(form
				.getTypeDevoirSel().getCategorie().getId()));
	}
	...
	public void saveLibelleTypeDevoir() {
		...
		try {
			...
			log.debug(
					"Erreur de sauvegarde du libellé de type de devoir : {0}",
					e.getMessage());
		}
	}
	...
	public void deleteTypeDevoir() {
		try {
			...
			initialiserListeLibelleTypeDevoir(obtenirSessionUtilisateurDTO()
					.getIdEtablissement());
		} catch (MetierException e) {
			log.debug("Erreur de destrcution du type de devoir : {0}",
					e.getMessage());
		}
	}
	...
	public void saveOuvertureEtablissement() {
		...
		final UtilisateurDTO utilisateurDTO = obtenirSessionUtilisateurDTO();
		final OuvertureQO ouvertureQO = new OuvertureQO(
				utilisateurDTO.getIdEtablissement(), vraiOuFauxOuvert);
		try {
			...
			log.debug(
					"Erreur de sauvegarde de l'ouverture établissement : {0}",
					e.getMessage());
		}
	}
	...
	/**
	 * Appelee lors de la validation de modification/ajout d'une plage horaire
	 * de la grille emploi du temps.
	 */
	public void validerModificationHorairePlage() {

		if (form.getPlageSelectedIndex() != null
				&& form.getPlageEditee() != null) {
			log.info(
					"validerModificationHorairePlage Selected index {0}, total grille {1}, debut {2}:{3}, fin {4}:{5}.  obj {6} obj {7}",
					form.getPlageSelectedIndex(), form.getHorairesCours()
							.size(), form.getPlageEditee().getHeureDebut(),
					form.getPlageEditee().getMinuteDebut(), form
							.getPlageEditee().getHeureFin(), form
							.getPlageEditee().getMinuteFin(),
	...
	/**
	 * Bascule dans la plage identifiee par plageSelectedIndex, les valeur de la
	 * plage editee.
	 */
	public void validerEditionHorairePlage() {
		...
		form.getHorairesCours().set(form.getPlageSelectedIndex(),
				form.getPlageEditee());
	...
	public void supprimerPlage() {
		...
		log.info(
				"supprimerPlage Selected index {0}, total grille {1}, debut {2}:{3}, fin {4}:{5}.  obj {6} obj {7}",
				form.getPlageSelectedIndex(), form.getHorairesCours().size(),
				form.getPlageEditee().getHeureDebut(), form.getPlageEditee()
						.getMinuteDebut(), form.getPlageEditee().getHeureFin(),
				form.getPlageEditee().getMinuteFin(), form.getHorairesCours()
