...
public class PeriodeEmploiDTO implements Serializable, Identifiable {
    ...
    /**
     * @param listePeriode
     *            une liste EN ORDRE plus tot vers plus tard.
     * @param date
     *            d
     * @return la période ou null
     */
    public static PeriodeEmploiDTO findPeriodePourDate(
            List<PeriodeEmploiDTO> listePeriode, Date date) {
        ...
        for (int i = listePeriode.size() - 1; i >= 0; --i) {
            if (DateUtils
                    .lessOrEquals(listePeriode.get(i).getDateDebut(), date)) {
